...
class Persona:
    def __init__(self, name, characteristics, personality):
        self.name = name
...
frank = Persona(
    name="Frank the Fitness Enthusiast",
...
# Randomly pick a number between 2 and 9
num_personas = random.randint(2, 9)

# Randomly select the chosen number of personas from the list
selected_personas = random.sample(personas, num_personas)

# Construct the prompt
prompt = f"A conversation among {num_personas} personas:\n"
for persona in selected_personas:
    prompt += f"\n{persona.name}:\nCharacteristics: {', '.join(persona.characteristics)}\nPersonality: {persona.personality}\n"

prompt += "\nThey are all sitting around a table, having a lively and engaging conversation. Write their dialogue below:"
